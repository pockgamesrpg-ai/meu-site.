...
function renderGames(filter = "all") {
  ...
  if (!grid) {
    console.error("❌ #gamesGrid não encontrado.");
    return;
  }
  ...
  grid.innerHTML = list.map((game, index) => `
    <article
      ...
      style="transition-delay:${Math.min(index * 0.05, 0.25)}s"
    >
...
async function initMercadoPago() {
  ...
  try {
    ...
    if (!response.ok) {
      ...
      throw new Error(
        data.error ||
        "Não foi possível buscar a Public Key."
      );
    ...
    if (
      ...
      throw new Error(
        "SDK Mercado Pago não carregado."
      );
...
async function getOfficialCart() {
...
async function renderPaymentBrick() {
  ...
  try {
    ...
    if (!container) {
      ...
      throw new Error(
        "Container do pagamento não encontrado."
      );
      ...
      callbacks: {
        ...
        onError: error => {
          ...
          paymentMessage(
            `
              <strong>
                ❌ O checkout do Mercado Pago encontrou um erro.
              </strong>

              <br><br>

              Abra o Console do navegador para ver os detalhes.
            `,
            true
          );
...
console.log(
  `🎮 BlueVault carregado com ${games.length} jogos`
);
